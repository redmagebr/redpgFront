...
this.rollIniciativa = function () {
    var inic = Math.floor(this.nivelAtual + (this.mainSheet.getField("Agilidade").getObject() / 2));
    var mod = this.mainSheet.getField("iniciativaBonus").getObject();
...
this.getValorOf = function (nome, loop) {
    ...
    if (atributoFixo !== -1) {
        result = [this.mainSheet.getField(atributos[atributoFixo]).getObject(), 1];
        if (half) result[0] = Math.floor(result[0]/2);
        return result;
    }
    
    var pericias = this.mainSheet.getField("Pericias").list;
    ...
    loop.push(nome);
    var rolagens = this.mainSheet.getField("rollEspRoll").list;
...
this.hideAutomaticos = function (sheet) {
    if (sheet.getField("Automatic").getObject()) {
        sheet.$visible.find(".notAutomaticOnly").hide();
    } else {
        sheet.$visible.find(".notAutomaticOnly").unhide();
    }
};
...
this.getTechAtrib = function (atrib) {
	...
	return this.mainSheet.fields[atributoNome + 'Nivel'].getObject();
    } else if (atrib === 6) {
        return this.sabedoriaDeCombate;
    } else if (atrib === 7) {
	return this.mainSheet.fields['DefesaNivel'].getObject();
    } else if (atrib === 8) {
	return this.mainSheet.fields['AtaqueNivel'].getObject();
    }
    return 0;
};
...
this.getAtributoTeste = function (atrib) {
	var atributos = ["Forca", "Constituicao", "Agilidade", "Carisma", "Sabedoria", "Inteligencia", "ForcaDeVontade"];
	return this.mainSheet.getField(atributos[atrib]).getObject();
};
...
this.calcCheckbox = {
    ...
    Announce : $(sheet.$visible.find('#dfsAnnounceWhenDone')[0])
};
...
this.element = {
    dfsFerramentas : $(sheet.$visible.find('#dfsFerramentas')[0]),
    dfsFerramentasOpenClose : $(sheet.$visible.find('#dfsFerramentasOpenClose')[0]),
    Sabedoria : $(sheet.$visible.find('#dfsSdCNivel')[0]),
    dfsIniciativaValue : $(sheet.$visible.find('#dfsIniciativaValue')[0]),
	InventarioAtual : $(sheet.$visible.find("#InventarioAtual")[0]),
	dfsExpTotal : $(sheet.$visible.find("#dfsExpTotal")[0]),
	dfsLevelupXP : $(sheet.$visible.find("#dfsLevelupXP")[0]),
	dfsExpTotalBar : $(sheet.$visible.find("#dfsExpTotalBar")[0]),
	dfsExpAtual : $(sheet.$visible.find("#dfsExpAtual")[0]),
	dfsExpAtualBar : $(sheet.$visible.find("#dfsExpAtualBar")[0]),
	NivelAtual : $(sheet.$visible.find("#NivelAtual")[0]),
	dfsSdCNivel : $(sheet.$visible.find("#dfsSdCNivel")[0]),
	dfsSdCExp : $(sheet.$visible.find("#dfsSdCExp")[0]),
	dfsResistenciaNivel : $(sheet.$visible.find("#dfsResistenciaNivel")[0]),
	dfsCura : $(sheet.$visible.find("#dfsCura")[0]),
	dfsStaminaMaxima : $(sheet.$visible.find("#dfsStaminaMaxima")[0]),
	sumAtributosTeste : $(sheet.$visible.find("#sumAtributosTeste")[0]),
	DesvantagensSum : $(sheet.$visible.find("#DesvantagensSum")[0]),
	VantagensSum : $(sheet.$visible.find("#VantagensSum")[0]),
	SumPericias : $(sheet.$visible.find("#SumPericias")[0]),
	dfsDiv : $(sheet.$visible.find("#dfsDiv")[0]),
	dfsMPForm : $(sheet.$visible.find("#dfsMPForm")[0]),
	dfsMPMotivo : $(sheet.$visible.find("#dfsMPMotivo")[0]),
	dfsExpForm : $(sheet.$visible.find("#dfsExpForm")[0]),
	dfsExpMotivo : $(sheet.$visible.find("#dfsExpMotivo")[0]),
	dfsCalculadoraDano : $(sheet.$visible.find("#dfsCalculadoraDano")[0]),
	dfsCalculadoraMP : $(sheet.$visible.find("#dfsCalculadoraMP")[0]),
	dfsCalculadoraExp : $(sheet.$visible.find("#dfsCalculadoraExp")[0])
};
...
this.getRD = function (atributo) {
	...
	var rd = this.mainSheet.fields[atributoNome + 'Nivel'].getObject();
	...
	// Achar RD específica
	rd += this.mainSheet.fields[atributoNome + 'RD'].getObject();
	
	// Somar RD Geral
	rd += this.mainSheet.fields['GeralRD'].getObject();
...
this.getRDBase = function () {
	return {
		Nome : "Sem Tipo",
		RD : this.getResistencia() + this.mainSheet.fields['GeralRD'].getObject()
	};
};