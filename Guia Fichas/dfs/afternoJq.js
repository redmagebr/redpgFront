...
this.sheet.addToggledEditListener({
	...
	handleEvent : function () {
		if (this.style.editing) {
			this.style.$visible.find('.diceButton').hide();
		} else {
			this.style.$visible.find('.diceButton').unhide();
		}
	}
});

var hideDices = function (row) {
	if (row.style.editing) {
		row.$visible.find('.diceButton').hide();
	} else {
		row.$visible.find('.diceButton').unhide();
	}
};

this.sheet.fields['Pericias'].addAddedRowListener(hideDices);
this.sheet.fields['Rolagens'].addAddedRowListener(hideDices);
this.sheet.fields['rollEspRoll'].addAddedRowListener(hideDices);
...
var resp = this.sheet.getField('rollEspRoll');
resp.$visible.on('newRow', bindREsp);

for (var i = 0; i < resp.sheets.length; i++) {
    bindREsp(null, resp.sheets[i]);
}
...
var efeitos = this.sheet.getField('Debuffs');
efeitos.addCreatedRowListener(bindBuffs);

for (var i = 0; i < efeitos.sheets.length; i++) {
    bindBuffs(null, efeitos.sheets[i]);
}
...
var rolagens = this.sheet.getField('Rolagens');
rolagens.addCreatedRowListener(bindRolagem);
rolagens.addCreatedRowListener(bindRolagemAtributosAutomaticos);

for (var i = 0; i < rolagens.sheets.length; i++) {
    bindRolagem(null, rolagens.sheets[i]);
    bindRolagemAtributosAutomaticos(null, rolagens.sheets[i]);
}
...
var pericias = this.sheet.getField('Pericias');
pericias.addCreatedRowListener(bindPericia);

for (var i = 0; i < pericias.sheets.length; i++) {
	bindPericia(null, pericias.sheets[i]);
}
...
this.sortTecnicas = function () {
	var newTechs = this.sheet.fields['Tecnicas'].getObject();
	...
	this.sheet.fields['Tecnicas'].storeValue(newTechs);
};
...
this.sortPericias = function () {
	var newList = this.sheet.fields['Pericias'].getObject();
	...
	this.sheet.fields['Pericias'].storeValue(newList);
};
...
this.sortEquipamentos = function () {
	var newList = this.sheet.fields['Equipamentos'].getObject();
	newList.sort(function (a,b) {
		if (a.Tipo !== b.Tipo) {
			return a.Tipo - b.Tipo;
		}
	...
	this.sheet.fields['Equipamentos'].storeValue(newList);
};
...
this.sortRolagens = function () {
	var newList = this.sheet.fields['Rolagens'].getObject();
	newList.sort(function (a,b) {
	...
	this.sheet.fields['Rolagens'].storeValue(newList);
};
...
this.sortRolagensEsp = function () {
	var newList = this.sheet.fields['rollEspRoll'].getObject();
	newList.sort(function (a,b) {
		if (a.Nome.toUpperCase() < b.Nome.toUpperCase()) {
			return -1;
		} else if (b.Nome.toUpperCase() < a.Nome.toUpperCase()) {
	...
	this.sheet.fields['rollEspRoll'].storeValue(newList);
};
...
this.sortVantagens = function (tipo) {
	var newList = this.sheet.fields[tipo].getObject();
	newList.sort(function (a,b) {
	...
	this.sheet.fields[tipo].storeValue(newList);
};
...
this.sortAnotacoes = function () {
	var newList = this.sheet.fields['Anotacoes'].getObject();
	newList.sort(function (a,b) {
	...
	this.sheet.fields['Anotacoes'].storeValue(newList);
};
...
this.sortInventario = function () {
	var newList = this.sheet.fields['Inventario'].getObject();
	newList.sort(function (a,b) {
            if (a.Nome.toUpperCase() < b.Nome.toUpperCase()) {
                    return -1;
            } else if (b.Nome.toUpperCase() < a.Nome.toUpperCase()) {
	...
	this.sheet.fields['Inventario'].storeValue(newList);
};
...
this.calcDano = function (dano, stamina) {
	...
	var info = {
		...
		oldHP : oldHP,
		newHP : newHP,
		nomePersonagem : this.sheet.nameField.getObject(),
...
sheet.fields['Tecnicas'].addCreatedRowListener(function (list, row) {
	row.addChangedListener(function (sheet) {
		var parent = sheet;
		var $parent = parent.$visible;
...
this.sumPericias = function (lastCount) {
	...
	var pericias = this.sheet.fields['Pericias'];
	var sum = 0;
...
this.sumVants = function (lastCount, type) {
	...
	var desvantagens = this.sheet.fields[type];
	var sum = 0;
...
this.fixExpBar = function () {
	...
	this.element['NivelAtual'].text(this.nivelAtual);
        this.element['dfsIniciativaValue'].text(Math.floor(this.nivelAtual + (this.sheet.getField("Agilidade").getObject() / 2)));
};
...
this.calcularPesos = function (lastCount) {
	...
	var bags = this.sheet.fields["Inventario"];
	var bag;
	var itens;
	var item;
	var k;
